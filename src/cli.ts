...
async function setupNextJs(cwd: string) {
  ...
  if (fs.existsSync(babelrcPath)) {
    ...
    if (content.includes('made-refine')) {
      console.log(pc.dim('  .babelrc — already configured'))
    } else {
      console.log(pc.yellow('  .babelrc already exists — please add "made-refine/babel" plugin manually'))
    }
  } else {
    ...
    console.log(pc.green(`  ✓ Created .babelrc`))
