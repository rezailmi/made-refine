...
import { getElementScale } from './utils/resize-geometry'
...
function getInsertIndex(siblings: HTMLElement[], insertBefore: HTMLElement | null): number | null {
...
function withAxisTranslate(baseTransform: string, axis: 'x' | 'y', value: number): string {
  const translate = axis === 'x' ? `translateX(${value}px)` : `translateY(${value}px)`
...
function tryReparent(
  ...
  const isSamePosition =
    target.container === originalParent && target.insertBefore === originalNextSibling
...
export function useMove({ onMoveComplete }: UseMoveOptions): UseMoveResult {
  ...
  const setReorderPreviewTransform = React.useCallback(
    (element: HTMLElement, transform: string) => {
      ...
      const originalTransition =
        reorderPreviewRef.current.get(element)?.transition ?? element.style.transition
  ...
  const applyReorderPreview = React.useCallback(
    (
      ...
      for (const [element, shift] of shiftedElements) {
        const baseTransform =
          reorderPreviewRef.current.get(element)?.transform ?? element.style.transform
  ...
  React.useEffect(() => {
    ...
    function handlePointerMove(e: PointerEvent) {
      ...
      const container = dragOptionsRef.current.constrainToOriginalParent
        ? originalParent
        : findContainerAtPoint(e.clientX, e.clientY, draggedElement, originalParent)
